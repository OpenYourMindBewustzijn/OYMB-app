...
const SUPABASE_URL = "https://euvwpdmrbsjqkluaeqpi.supabase.co";
const SUPABASE_KEY = "sb_publishable_wVoqjZAfae3OO715_wIN5w_KbGytRMC";
...
const MODULES = [
  ...
  {id:9,emoji:"🦁",color:"#E8A44A",title:"Hoe dieren ons de weg wijzen: uit de freeze",
   ...
   sections:[
     {t:"Wat dieren ons leren",b:"Kijk eens naar een zebra die net is ontsnapt aan een leeuw. Wat doet die zebra?\n\nHij stopt. Hij trilt. Hij schudt zijn hele lichaam uit. En dan... graast hij gewoon weer verder.\n\nDat is geen zwakte. Dat is biologische intelligentie.\n\nDieren ontladen spanning automatisch na een bedreiging. Hun zenuwstelsel weet precies wat het moet doen: de opgebouwde spanning afvoeren via beweging en trillen.\n\nWij mensen doen dat niet meer. We houden ons groot. We gaan weer aan het werk. We nemen de spanning mee."},
     {t:"Freeze: het meest misbegrepen overlevingsmechanisme",b:"Wanneer een dier wordt aangevallen en niet kan vluchten of vechten... speelt het dood. Het lichaam bevriest volledig.\n\nBij mensen ziet freeze er zo uit:\n- Je wilt iets doen maar kunt jezelf niet in beweging krijgen\n- Je ligt op de bank terwijl je weet dat je moet opstaan\n- Je bent aanwezig maar ook niet echt er\n- Je voelt je leeg, vlak, verdoofd\n\nDit is geen luiheid. Dit is een zenuwstelsel dat heeft besloten: stilstand is de veiligste optie."},
     {t:"Waarom uit freeze komen anders werkt",b:"Je kunt jezelf niet denken uit freeze. Freeze is een lichamelijke staat, geen mentale keuze.\n\nWat wel helpt:\n- Kleine, zachte bewegingen\n- Warmte: een warme douche, een deken, warme thee\n- Zintuiglijke prikkels: een geur, een textuur, een geluid\n- Heel langzaam ademen\n- De grond onder je voeten voelen\n\nHet gaat erom je zenuwstelsel zacht te laten weten: het gevaar is voorbij."},
     {t:"Het trillingsreflex: de sleutel",b:"TRE -- Tension & Trauma Releasing Exercises -- is gebaseerd op precies dit principe. Het activeert de natuurlijke trillingsreflex die dieren automatisch gebruiken.\n\nWanneer je lichaam begint te trillen tijdens TRE, is dat geen zwakte. Dat is je zenuwstelsel dat doet wat het altijd al heeft willen doen: ontladen.\n\nNa die ontlading voelen veel vrouwen een diepe rust die ze al jaren niet meer hebben gevoeld."},
   ],
   exercises:[
     {t:"Uit freeze stappen: de 4-stappen methode",inst:"Stap 1: Erken het zonder oordeel. Zeg: Mijn lichaam is nu in freeze. Dat is oké.\n\nStap 2: Voel de grond. Zet beide voeten plat op de vloer. Druk ze bewust naar beneden.\n\nStap 3: Een kleine beweging. Beweeg alleen een vinger. Dan je hand. Dan je pols. Heel langzaam.\n\nStap 4: Adem verlengd uit. Adem in 4 tellen. Adem uit 8 tellen. Herhaal 5 keer.",prompts:["Hoe voelde ik mij voor de oefening?","Welke stap hielp het meest?","Wat merkte ik in mijn lichaam?"]},
     {t:"Het dierenschud",inst:"Ga stevig staan. Voeten op schouderbreedte.\n\nBegin bij je voeten. Maak kleine trilbewegingen. Laat het langzaam opstijgen: enkels, knieen, heupen, buik, schouders, armen, handen.\n\nDoe dit 3 minuten. Niet mooi. Niet gecontroleerd. Gewoon schudden.\n\nStop dan. Sta stil. Adem. Voel na.",prompts:["Hoe voelde het schudden?","Wat veranderde er in mijn lichaam?"]},
     {t:"Warmte als veiligheid",inst:"Wanneer je in diepe freeze zit en beweging te veel voelt:\n\n- Maak een kop warme thee\n- Wikkel een deken om je heen\n- Neem een warme douche\n- Leg een warmwaterkruik op je buik\n\nZeg innerlijk: Ik ben veilig. Mijn lichaam mag ontdooien.",prompts:["Welke warmtebron hielp mij vandaag?","Hoe voelde mijn lichaam voor en na?"]},
   ],
...
function LoginForm({ onLogin }) {
  ...
  const login = async () => {
    ...
    if (err) { setError("E-mailadres of wachtwoord is onjuist. Controleer je gegevens of neem contact op met Wendy."); setLoading(false); return; }
  ...
  return (
    <div style={{ position:"relative", zIndex:2, maxWidth:380, width:"100%" }}>
      ...
      <p style={{ ...s.mu, marginBottom:28, fontSize:13 }}>Log in met je persoonlijke gegevens.</p>
      <div style={{ background:T.dark, border:"1px solid rgba(249,132,229,0.2)", borderRadius:16, padding:"24px 20px", marginBottom:16, textAlign:"left" }}>
        <div style={{ fontSize:11, color:T.muted, letterSpacing:"0.15em", textTransform:"uppercase", marginBottom:8 }}>E-mailadres</div>
        <input type="email" placeholder="jouw@email.nl" value={email}
          onChange={e => { setEmail(e.target.value); setError(""); }}
          onKeyDown={e => e.key === "Enter" && login()}
          style={s.inp}/>
        <div style={{ fontSize:11, color:T.muted, letterSpacing:"0.15em", textTransform:"uppercase", marginBottom:8 }}>Wachtwoord</div>
        <input type="password" placeholder="Jouw persoonlijke wachtwoord" value={password}
          onChange={e => { setPassword(e.target.value); setError(""); }}
          onKeyDown={e => e.key === "Enter" && login()}
          style={{ ...s.inp, marginBottom:16 }}/>
        {error && <p style={{ fontSize:12, color:T.rose, marginBottom:12 }}>{error}</p>}
        <button style={{ ...s.btn(), width:"100%", justifyContent:"center", opacity:loading ? 0.7 : 1 }} onClick={login} disabled={loading}>
          {loading ? "Even geduld..." : "Inloggen"}
        </button>
      </div>
      <p style={{ fontSize:12, color:T.muted, marginBottom:10 }}>Nog geen toegang?</p>
      ...
      <p style={{ fontSize:10, color:T.muted, marginTop:14, lineHeight:1.6 }}>Je gegevens zijn veilig en worden nooit gedeeld.</p>
    </div>
  );
}

// ─── WENDY DASHBOARD ──────────────────────────────────────────────────────────
function WendyPanel({ onBack }) {
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  ...
  const addUser = async () => {
    if (!email.trim() || !password.trim() || !name.trim()) { setMsg("Vul alle velden in."); return; }
    ...
    else { setMsg("Klant " + name + " aangemaakt! Stuur hen de inloggegevens via WhatsApp."); setEmail(""); setPassword(""); setName(""); }
    setLoading(false);
  };

  return (
    <div style={{ minHeight:"100svh", background:T.black, color:T.cream, padding:"40px 20px" }}>
      <style>{G}</style>
      <div style={{ maxWidth:480, margin:"0 auto" }}>
        <button style={{ ...s.sm, marginBottom:24 }} onClick={onBack}>← Terug naar app</button>
        <span style={s.ey}>Wendy — Beheerderspaneel</span>
        <h2 style={{ ...s.h2 }}>Klanten <em style={s.em}>beheren</em></h2>
        <div style={s.card({ border:"1px solid rgba(249,132,229,0.2)", marginBottom:20 })}>
          <div style={{ fontSize:12, color:T.rose, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:14 }}>Nieuwe klant toevoegen</div>
          <div style={{ fontSize:11, color:T.muted, marginBottom:6 }}>Naam klant</div>
          <input style={s.inp} placeholder="Bijv. Lisa de Vries" value={name} onChange={e => setName(e.target.value)}/>
          <div style={{ fontSize:11, color:T.muted, marginBottom:6 }}>E-mailadres klant</div>
          <input style={s.inp} type="email" placeholder="lisa@email.nl" value={email} onChange={e => setEmail(e.target.value)}/>
          <div style={{ fontSize:11, color:T.muted, marginBottom:6 }}>Wachtwoord (kies iets unieks)</div>
          <input style={s.inp} type="text" placeholder="Bijv. Lisa2026!" value={password} onChange={e => setPassword(e.target.value)}/>
          {msg && <p style={{ fontSize:12, color: msg.includes("Fout") ? T.rose : T.sage, marginBottom:10 }}>{msg}</p>}
          <button style={{ ...s.btn(), width:"100%", justifyContent:"center", opacity:loading ? 0.7 : 1 }} onClick={addUser} disabled={loading}>
            {loading ? "Bezig..." : "Klant toevoegen"}
          </button>
        </div>
        <div style={s.card()}>
          <div style={{ fontSize:12, color:T.rose, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:10 }}>Stuur dit WhatsApp-bericht naar je klant</div>
          <p style={{ ...s.mu, fontSize:13, whiteSpace:"pre-line" }}>Hoi [naam]! Je hebt toegang tot de OYMB app. Log in via:
...
export default function App() {
  ...
  if (screen === "loading") return (
    <div style={{ minHeight:"100svh", background:T.black, display:"flex", alignItems:"center", justifyContent:"center" }}>
      <style>{G}</style>
  ...
  if (screen === "login") return (
    <div style={{ minHeight:"100svh", background:T.black, color:T.cream, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", textAlign:"center", padding:"60px 24px", position:"relative", overflow:"hidden" }}>
      ...
      <LoginForm onLogin={(name, email) => { setUserName(name); setUserEmail(email); setScreen("app"); scrollTop(); }}/>
    </div>
  );

  if (screen === "wendy") return <WendyPanel onBack={() => setScreen("app")}/>;
  ...
  if (screen === "hero") return (
    <div style={{ minHeight:"100svh", background:T.black, color:T.cream, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", textAlign:"center", padding:"80px 24px", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ position:"relative", zIndex:2 }}>
        ...
        <button style={s.btn()} onClick={() => { setScreen("login"); scrollTop(); }}>Start het programma</button>
      </div>
    </div>
  );
